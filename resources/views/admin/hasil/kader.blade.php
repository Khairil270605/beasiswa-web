...
@section('title', 'Hasil Perhitungan SAW - Beasiswa Kader')
...
:root {
    ...
    --kader-accent: #3498db;       /* Accent khusus Kader - Biru (untuk referensi saja) */
}
...
.btn-print {
    background-color: var(--kader-accent);
    color: white;
}
...
.stat-icon.blue { color: var(--kader-accent); }
.stat-icon.gold { color: var(--secondary-color); }
.stat-icon.green { color: var(--success-color); }
...
.nilai-bobot { color: var(--success-color); }
.nilai-akhir { 
    color: var(--primary-color); 
    font-weight: bold; 
    font-size: 1.1em;
...
<div class="saw-container">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                <h1 class="page-title">
                    ...
                    Hasil Perhitungan SAW - Beasiswa Kader
                </h1>
                <p class="page-subtitle">
                    Perangkingan calon penerima beasiswa kader berdasarkan metode Simple Additive Weighting
                </p>
    ...
    <div class="action-buttons fade-in">
        <a href="{{ route('admin.kader') }}" class="btn-action btn-primary">
            <i class="fas fa-calculator"></i>
            Hitung Ulang
        </a>
        <a href="{{ route('admin.alternatif.index') }}" class="btn-action btn-success">
            <i class="fas fa-users"></i>
            Data Pendaftar Kader
        </a>
        <a href="{{ route('admin.kriteria.kader') }}" class="btn-action btn-warning">
            <i class="fas fa-cogs"></i>
            Kelola Kriteria Kader
        </a>
        <a href="{{ route('admin.dhuafa') }}" class="btn-action btn-info">
    ...
    <div class="stats-cards fade-in">
        <div class="stat-card">
            ...
            <div class="stat-label">Total Peserta Kader</div>
        </div>
        <div class="stat-card">
    ...
    <div class="calculation-info fade-in">
        <h4><i class="fas fa-info-circle"></i>Informasi Perhitungan Beasiswa Kader</h4>
        <p><strong>Metode:</strong> Simple Additive Weighting (SAW)</p>
        <p><strong>Kategori:</strong> Beasiswa Kader - untuk mahasiswa yang aktif dalam kegiatan organisasi dan kepemimpinan</p>
    ...
    <div class="results-container fade-in">
        <div class="results-header">
            <h3 class="results-title">
                ...
                Peringkat Penerima Beasiswa Kader
            </h3>
        </div>
...
@forelse($hasil as $item)
    @php
        ...
        } elseif ($statusBeasiswa === 'tidak_diterima') {
            $rowClass = 'table-danger';
        }
    @endphp

    <tr class="result-row {{ $rowClass }}" data-rank="{{ $item['ranking'] }}">
        {{-- PERINGKAT --}}
        ...
        <td data-label="Peserta">
            <div class="participant-info">
                ...
                <div class="participant-details">
                    ...
                    @if(!empty($item['alternatif']['jurusan']))
                        <br>
                        <i class="fas fa-graduation-cap"></i>
                        {{ $item['alternatif']['jurusan'] }}
                    @endif
                </div>
            </div>
        </td>
        ...
        <td data-label="Status Beasiswa">
            <form method="POST"
                ...
                <select name="status_beasiswa"
                    ...
                    <option value="menunggu" {{ $statusBeasiswa=='menunggu'?'selected':'' }}>
                        Menunggu
                    </option>
                    <option value="diterima" {{ $statusBeasiswa=='diterima'?'selected':'' }}>
                        ✅ Diterima
                    </option>
                    <option value="tidak_diterima" {{ $statusBeasiswa=='tidak_diterima'?'selected':'' }}>
                        ❌ Tidak Diterima
                    </option>
                </select>

                {{-- supaya catatan tidak ke-reset --}}
                <input type="hidden"
                       name="catatan_beasiswa"
                       value="{{ $item['alternatif']['catatan_beasiswa'] ?? '' }}">
            </form>
        ...
        <td colspan="6">
            <div class="empty-state">
                ...
                <div class="empty-title">
                    Belum Ada Hasil Perhitungan Beasiswa Kader
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    rows.forEach((row, index) => {
        ...
        if (rank <= 3) {
            setTimeout(() => {
                row.style.background = 'linear-gradient(90deg, rgba(52, 152, 219, 0.1) 0%, #ffffff 100%)';
...
function exportResults() {
    ...
    link.setAttribute("download", `hasil_saw_kader_${new Date().toISOString().slice(0,10)}.csv`);
...
function loadDetailData(alternatifId, namaAlternatif) {
    ...
    let html = `
        <div class="detail-section">
            <h6><i class="fas fa-user-graduate mr-2"></i>Informasi Peserta Kader</h6>
            <p><strong>Nama:</strong> ${detailData.nama}</p>
            <p><strong>Kategori:</strong> <span class="badge badge-primary">Kader</span></p>
    ...
    html += `
        ...
        <div class="detail-section">
            ...
            <div class="calculation-step">
                <div class="step-title">3. Hasil Akhir untuk Kategori Kader</div>
...
/*
function loadDetailData(alternatifId, namaAlternatif) {
    fetch(`/admin/hasil/kader/detail/${alternatifId}`)
...
console.log('🏆 SAW Kader Results page loaded successfully!');
